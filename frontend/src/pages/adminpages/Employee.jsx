...
export default function Employee() {
  ...
  // Create branch options from employees
  const branchOptions = [
    ...new Set(allEmployees.map((e) => e.user?.name).filter(Boolean)),
  ];
  ...
  const filteredEmployees = useMemo(() => {
    ...
    return allEmployees.filter((emp) => {
      ...
      const matchesBranch = userFilter ? branch === userFilter : true;
  ...
  return (
    <AdminLayout>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
        ...
        <input
          type="text"
          placeholder="Search by name, phone, designation or branch..."
          value={search}
          onChange={(e) => {
        ...
        <select
          ...
          className="w-full md:w-60 px-4 py-2 rounded-xl border border-gray-300"
        >
